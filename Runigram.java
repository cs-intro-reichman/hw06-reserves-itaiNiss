...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		// Color[][] image = read("cake.ppm");
		// Color[][] image2 = read("thor.ppm");
		// morph(image, image2, 100);
	...
	private static void print(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[i].length; j++) {
				System.out.print(String.format("(%3s, %3s, %3s)", image[i][j].getRed(), image[i][j].getGreen(),
						image[i][j].getBlue()) + " ");
				System.out.print("   ");
			}
			System.out.println();
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < ans.length; i++) {
			for (int j = 0; j < ans[i].length; j++) {
				ans[i][j] = image[i][ans[i].length - j - 1];
			}
		}
		return ans;
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < ans.length; i++) {
			for (int j = 0; j < ans[i].length; j++) {
				ans[i][j] = image[ans.length - i - 1][j];
			}
		}
		return ans;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color ans = new Color((int) red, (int) green, (int) blue);
		return ans;
	}
